...
class OccurrenceOrderer:
    """Imposes an ordering on an office according to precedence in occurrence.

    Wrapper around an office that imposes an ordering for sorting according
    to precedence in occurrence.  Intended to be used as a sort key.  Smaller
    is better.
    """
    ...
    @property
    def rank(self):
        return self._office.rank

    def __lt__(self, other):
        return self.rank < other.rank


class CommemorationOrderer:
    """Imposes an ordering on an office according to order of commemorations.

    Wrapper around an office that imposes an ordering for sorting according
    to precedence in commemoration.  Intended to be used as a sort key.
    Smaller is better.
    """
    def __init__(self, office):
        self._office = office
...
class CalendarResolver:
    ...
    def calpoint_offices(self, calpoint):
        ...
        if calentry in self._data_map:
            ...
            for descriptor in self._data_map[calentry]:
                ...
                office_list.append(offices.Feast(root, rank))
            return office_list
    ...
    def resolve_commemorations(self, occurring, concurring=[]):
        ...
        return sorted(occurring + concurring, key=CommemorationOrderer)
